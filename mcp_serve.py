"""
Mavis MCP Server — expose messaging conversations as MCP tools.
...
Plus: channels_list (Mavis-specific extra)

Usage:
    mavis mcp serve
    mavis mcp serve --verbose
...
logger = logging.getLogger("mavis.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using the active Mavis home."""
    try:
        from hermes_constants import get_hermes_home
        return get_hermes_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("MAVIS_HOME", Path.home() / ".mavis")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        ...
        directory_file = Path(
            os.environ.get("MAVIS_HOME", Path.home() / ".mavis")
...
class EventBridge:
    ...
    This is the Mavis equivalent of OpenClaw's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            ...
            db_file = Path(os.environ.get("MAVIS_HOME", Path.home() / ".mavis")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Mavis MCP server with all tools registered."""
    if not _MCP_SERVER_AVAILABLE:
        raise ImportError(
            "MCP server requires the 'mcp' package. "
            "Install with: pip install 'mavis-agent[mcp]'"
        )

    mcp = FastMCP(
        "mavis",
        instructions=(
            "Mavis messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Mavis MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
