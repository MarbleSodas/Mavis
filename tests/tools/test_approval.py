...
class TestTeePattern:
    ...
    def test_tee_hermes_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee ~/.mavis/.env")
        assert dangerous is True
        assert key is not None

    def test_tee_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x | tee $MAVIS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestSensitiveRedirectPattern:
    ...
    def test_redirect_to_custom_hermes_home_env(self):
        dangerous, key, desc = detect_dangerous_command("echo x > $MAVIS_HOME/.env")
        assert dangerous is True
        assert key is not None
...
class TestGatewayProtection:
    ...
    def test_gateway_run_with_disown_detected(self):
        cmd = "kill 1605 && cd ~/.mavis/hermes-agent && source venv/bin/activate && python -m hermes_cli.main gateway run --replace &disown; echo done"
    ...
    def test_systemctl_restart_not_flagged(self):
        ...
        cmd = "systemctl --user restart mavis-gateway"
        dangerous, key, desc = detect_dangerous_command(cmd)
        assert dangerous is False
