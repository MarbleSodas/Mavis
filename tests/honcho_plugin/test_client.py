...
class TestHonchoClientConfigDefaults:
    def test_default_values(self):
        ...
        assert config.host == "mavis"
        assert config.workspace_id == "mavis"
...
class TestFromGlobalConfig:
    ...
    def test_context_tokens_host_block_wins(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "contextTokens": 1000,
            "hosts": {"mavis": {"contextTokens": 2000}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_recall_mode_from_config(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "recallMode": "tools",
            "hosts": {"mavis": {"recallMode": "context"}},
        }))
        config = HonchoClientConfig.from_global_config(config_path=config_file)
    ...
    def test_base_url_not_read_from_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            ...
            "hosts": {"mavis": {"baseUrl": "http://host-block:9001"}},
        }))
...
class TestResolveSessionName:
    ...
    def test_per_repo_uses_git_root(self):
        ...
        with patch.object(
            HonchoClientConfig, "_git_repo_name", return_value="mavis-agent"
        ):
            result = config.resolve_session_name("/home/user/mavis-agent/subdir")
        assert result == "mavis-agent"
...
class TestResolveConfigPath:
    def test_prefers_mavis_home_when_exists(self, tmp_path):
        mavis_home = tmp_path / "mavis"
        mavis_home.mkdir()
        local_cfg = mavis_home / "honcho.json"
        local_cfg.write_text('{"apiKey": "local"}')

        with patch.dict(os.environ, {"MAVIS_HOME": str(mavis_home)}):
    ...
    def test_falls_back_to_global_when_no_local(self, tmp_path):
        mavis_home = tmp_path / "mavis"
        mavis_home.mkdir()
        # No honcho.json in MAVIS_HOME — also isolate ~/.mavis so
    ...
    def test_falls_back_to_global_without_mavis_home_env(self, tmp_path):
        fake_home = tmp_path / "fakehome"
        fake_home.mkdir()
...
class TestResolveActiveHost:
    def test_default_returns_mavis(self):
        with patch.dict(os.environ, {}, clear=True):
            os.environ.pop("MAVIS_HONCHO_HOST", None)
            os.environ.pop("MAVIS_HOME", None)
            assert resolve_active_host() == "mavis"

    def test_explicit_env_var_wins(self):
        with patch.dict(os.environ, {"MAVIS_HONCHO_HOST": "mavis.coder"}):
            assert resolve_active_host() == "mavis.coder"
    ...
    def test_default_profile_returns_mavis(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAVIS_HONCHO_HOST", None)
            with patch("hermes_cli.profiles.get_active_profile_name", return_value="default"):
                assert resolve_active_host() == "mavis"

    def test_custom_profile_returns_mavis(self):
        with patch.dict(os.environ, {}, clear=False):
            os.environ.pop("MAVIS_HONCHO_HOST", None)
...
class TestProfileScopedConfig:
    def test_from_env_uses_profile_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="mavis.coder")
        assert config.host == "mavis.coder"
        assert config.workspace_id == "mavis"  # shared workspace
        assert config.ai_peer == "mavis.coder"

    def test_from_env_default_workspace_preserved_for_default_host(self):
        with patch.dict(os.environ, {"HONCHO_API_KEY": "key"}):
            config = HonchoClientConfig.from_env(host="mavis")
        assert config.host == "mavis"
        assert config.workspace_id == "mavis"
    ...
    def test_from_global_config_reads_profile_host_block(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "shared-key",
            "hosts": {
                "mavis": {"aiPeer": "mavis", "peerName": "alice"},
                "mavis.coder": {
                    "aiPeer": "mavis.coder",
        ...
        config = HonchoClientConfig.from_global_config(
            host="mavis.coder", config_path=config_file,
        )
        assert config.host == "mavis.coder"
    ...
    def test_from_global_config_auto_resolves_host(self, tmp_path):
        ...
        config_file.write_text(json.dumps({
            "apiKey": "key",
            "hosts": {
                "mavis.dreamer": {"peerName": "dreamer-user"},
            },
        }))
        with patch("plugins.memory.honcho.client.resolve_active_host", return_value="mavis.dreamer"):
            config = HonchoClientConfig.from_global_config(config_path=config_file)
        assert config.host == "mavis.dreamer"
...
class TestObservationModeMigration:
    ...
    def test_existing_config_defaults_to_unified(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"mavis": {"enabled": True, "aiPeer": "mavis"}},
        }))
    ...
    def test_explicit_directional_respected(self, tmp_path):
        ...
        cfg_file.write_text(json.dumps({
            "apiKey": "k",
            "hosts": {"mavis": {"enabled": True, "observationMode": "directional"}},
        }))
        cfg = HonchoClientConfig.from_global_config(config_path=cfg_file)
        assert cfg.observation_mode == "directional"
