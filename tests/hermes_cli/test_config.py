...
def _clear_mavis_home_env(monkeypatch):
...
class TestGetHermesHome:
    ...
    def test_legacy_env_does_not_override(self):
        with patch.dict(os.environ, {"HERMES_HOME": "/legacy/path"}, clear=False):
            os.environ.pop("MAVIS_HOME", None)
            home = get_hermes_home()
            assert home == Path.home() / ".mavis"

    def test_legacy_env_does_not_beat_explicit_mavis_home(self, tmp_path):
        with patch("pathlib.Path.home", return_value=tmp_path), patch.dict(os.environ, {}, clear=True):
            os.environ["MAVIS_HOME"] = str(tmp_path / ".mavis-custom")
            os.environ["HERMES_HOME"] = str(tmp_path / ".hermes-profile")
            assert get_hermes_home() == tmp_path / ".mavis-custom"

    def test_does_not_import_legacy_home_automatically(self, tmp_path):
...
class TestEnsureHermesHome:
    def test_creates_subdirs(self, tmp_path):
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path)}):
            ensure_hermes_home()
...
class TestLoadConfigDefaults:
    def test_returns_defaults_when_no_file(self, tmp_path):
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path)}):
            ...
            assert config["voice"]["activation_mode"] == "hotword"
            assert config["voice"]["wake_phrase"] == "hey mavis"
            assert config["voice"]["ready"] is False
            assert config["stt"]["provider"] == "local"
            assert config["stt"]["local"]["model"] == "small"
...
class TestSaveEnvValueSecure:
    ...
    def test_save_env_value_updates_process_environment(self, tmp_path):
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path)}, clear=False):
...
class TestRemoveEnvValue:
    def test_removes_key_from_env_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path), "KEY_B": "value_b"}):
    ...
    def test_clears_os_environ(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path), "MY_KEY": "my_value"}):
    ...
    def test_handles_missing_env_file(self, tmp_path):
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path), "GHOST_KEY": "ghost"}):
    ...
    def test_clears_os_environ_even_when_not_in_file(self, tmp_path):
        ...
        with patch.dict(os.environ, {"MAVIS_HOME": str(tmp_path), "ORPHAN_KEY": "orphan"}):
