...
Handles: mavis honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "mavis" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all Mavis profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'mavis honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'mavis honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `mavis update` -- no output, no exceptions.
    """
    try:
        from hermes_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current Mavis profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $MAVIS_HOME/honcho.json so each profile/instance gets
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives Mavis persistent cross-session memory.")
    ...
    host_config = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'mavis honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = host_config.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        host_config["peerName"] = new_peer

    current_ai = host_config.get("aiPeer") or cfg.get("aiPeer", HOST)
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        host_config["aiPeer"] = new_ai

    current_workspace = host_config.get("workspace") or cfg.get("workspace", HOST)
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        host_config["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = host_config.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        host_config["observationMode"] = new_obs
    else:
        host_config["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(host_config.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        host_config["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        host_config["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = host_config.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        host_config["recallMode"] = new_recall

    # --- 7. Session strategy ---
    current_strat = host_config.get("sessionStrategy") or cfg.get("sessionStrategy", "per-directory")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        host_config["sessionStrategy"] = new_strat

    host_config["enabled"] = True
    host_config.setdefault("saveMessages", True)
    ...
    print("\n  Other commands:")
    print("    mavis honcho status     -- show full config")
    print("    mavis honcho mode       -- change recall/observation mode")
    print("    mavis honcho tokens     -- tune context and dialectic budgets")
    print("    mavis honcho peer       -- update peer names")
    print("    mavis honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active Mavis profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: mavis honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'mavis honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: mavis honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        mavis = hosts.get(_host_key(), {})
        user = mavis.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = mavis.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = mavis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = mavis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    Mavis' identity in Honcho. Seed with 'mavis honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: mavis honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = mavis.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = mavis.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = mavis.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. Mavis asks Honcho's AI peer a question")
        ...
        print("\n  Set with: mavis honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'mavis honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'mavis honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    mavis honcho identity --show        — show both peer representations")
        print("    mavis honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → Mavis + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → Mavis\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives Mavis persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  mavis honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'mavis honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'mavis honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  mavis honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'mavis'")
        print("  with Honcho configured and no prior session history.")
        print("  (Mavis calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    mavis honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'mavis honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In Mavis, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'mavis honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    mavis honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  mavis honcho identity <path/to/SOUL.md>")
    ...
    print("    Mavis:   cloud-backed Honcho peers. Files can stay on disk as source")
    ...
    print("    Mavis:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    Mavis:   Honcho observes every message and updates representations")
    ...
    print("    Mavis:   per-session by default — each run gets its own session")
    print("              Map a custom name:  mavis honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. mavis honcho setup              — configure API key (required)")
        print("  2. mavis honcho migrate            — re-run this walkthrough")
    else:
        print("  1. mavis honcho status             — verify Honcho connection")
        print("  2. mavis                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. mavis honcho identity --show    — verify AI peer representation")
        print("  4. mavis honcho tokens             — tune context and dialectic budgets")
        print("  5. mavis honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'mavis memory setup'...\n")
...
def register_cli(subparser) -> None:
    """Build the ``mavis honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``mavis honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to mavis memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to Mavis Honcho",
    )
