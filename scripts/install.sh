...
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/MarbleSodas/Mavis/main/scripts/install.sh | bash
#
# Or with options:
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  --dir PATH     Installation directory (default: ~/.mavis/mavis-agent)"
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  irm https://raw.githubusercontent.com/MarbleSodas/Mavis/main/scripts/install.ps1 | iex"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_node() {
    ...
    log_info "Extracting to ~/.mavis/node/..."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Mavis itself does not require or retain root access."
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$HOME/.local/bin$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Mavis — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
...
copy_config_templates() {
    ...
    if [ ! -f "$HERMES_HOME/.env" ]; then
        ...
        log_info "~/.mavis/.env already exists, keeping it"
    fi
    ...
    if [ ! -f "$HERMES_HOME/config.yaml" ]; then
        ...
        log_info "~/.mavis/config.yaml already exists, keeping it"
    fi
...
-->
SOUL_EOF
        log_success "Created ~/.mavis/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.mavis/"

    # Seed bundled skills into ~/.mavis/skills/ (manifest-based, one-time per skill)
    log_info "Syncing bundled skills to ~/.mavis/skills/ ..."
...
run_setup_wizard() {
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Setup wizard skipped (no terminal available). Run 'mavis setup' after install."
        return 0
    fi
    ...
    # Run mavis setup using the venv Python directly (no activation needed).
...
maybe_start_gateway() {
    ...
    log_info "The gateway needs to be running for Mavis to send/receive messages."
    ...
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            ...
            log_info "Running 'mavis whatsapp' to pair via QR code..."
            echo ""
    ...
    if ! [ -e /dev/tty ]; then
        log_info "Gateway setup skipped (no terminal available). Run 'mavis gateway install' later."
        return 0
    fi
    ...
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        ...
        if command -v systemctl &> /dev/null; then
            ...
            if $HERMES_CMD gateway install 2>/dev/null; then
                ...
                log_warn "Systemd install failed. You can start manually: mavis gateway"
            fi
        else
            ...
            log_info "To restart later: mavis gateway"
        fi
    else
        log_info "Skipped. Start the gateway later with: mavis gateway"
    fi
}
...
print_success() {
    ...
    # Show file locations
    echo -e "${CYAN}${BOLD}📁 Your files (all in ~/.mavis/):${NC}"
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    ~/.mavis/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  ~/.mavis/.env"
    echo -e "   ${YELLOW}Data:${NC}      ~/.mavis/cron/, sessions/, logs/"
    echo -e "   ${YELLOW}Code:${NC}      ~/.mavis/mavis-agent/"
    echo ""
    ...
    echo ""
    echo -e "   ${GREEN}mavis${NC}              Start chatting"
    echo -e "   ${GREEN}mavis setup${NC}        Configure API keys, voice, and settings"
    echo -e "   ${GREEN}mavis config${NC}       View/edit configuration"
    echo -e "   ${GREEN}mavis config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}mavis gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}mavis update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    echo -e "${YELLOW}⚡ Reload your shell to use 'mavis' command:${NC}"
    echo ""
    LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
