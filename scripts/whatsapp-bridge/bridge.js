 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.mavis/whatsapp/session
 */
...
const MAVIS_HOME = process.env.MAVIS_HOME || path.join(process.env.HOME || '~', '.mavis');
const SESSION_DIR = getArg('session', path.join(MAVIS_HOME, 'whatsapp', 'session'));
const IMAGE_CACHE_DIR = path.join(MAVIS_HOME, 'image_cache');
const DOCUMENT_CACHE_DIR = path.join(MAVIS_HOME, 'document_cache');
const AUDIO_CACHE_DIR = path.join(MAVIS_HOME, 'audio_cache');
const PAIR_ONLY = args.includes('--pair-only');
const WHATSAPP_MODE = getArg('mode', process.env.WHATSAPP_MODE || 'self-chat'); // "bot" or "self-chat"
const ALLOWED_USERS = parseAllowedUsers(process.env.WHATSAPP_ALLOWED_USERS || '');
const DEFAULT_REPLY_PREFIX = '⚕ *Mavis*\n────────────\n';
...
async function startSocket() {
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore Mavis's own reply messages in self-chat mode to avoid loops.
